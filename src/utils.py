...
import datetime
from ResnetModel import HashingModel, CLIPModel
import pretrainedmodels

from dataset_chair import CreateDataset_Sketchy,ShoeDataset
...
def get_train_args(passed_args=None):
    ...
    parser.add_argument('--dataset', '-d',
                        default='sketchy2',
                        type=str, choices=['sketchy', 'sketchy2', 'tuberlin', 'shoe'],
                        help='dir of model checkpoint (default: none)')
    ...
    parser.add_argument('--base-lr-mult', default=0.1, type=float,
                        metavar='LR', help='0.01 decay factor for backbone params')
    ...
    parser.add_argument('--tri-lambda', metavar='LAMBDA', default=1, type=float,
    ...
    parser.add_argument('--loss', metavar='LAMBDA', default='mathm', type=str,
                        help='loss type')
    ...
    parser.add_argument('--fix_margin', metavar='LAMBDA', default='fix', type=str,
                        help='loss type')


    parser.add_argument('--coordinate',
                        default='ShoeV2_Coordinate',
                        type=str,
                        help='dir of model checkpoint (default: none)')
    parser.add_argument('--roor_dir',
                        default='../dataset/ShoeV2',
                        type=str, metavar='PATH',
                        help='dir of model checkpoint (default: none)')
    ...
    '''
    parser.add_argument('--metircloss', metavar='LAMBDA', default='mathm', type=str,choice=['sketchy', 'sketchy2', 'tuberlin', 'shoe'],
...
def adjust_learning_rate(epoch, warmup_epochs=2, mode='exp', max_epoch=40, fix_epochs=0):  #目前max25好
    lr = 1
...
def load_data(args):
    ...
    if args.dataset == 'tuberlin':
        ...
        #有pht_val吗---没有，但是看起来和train是同样的类
        '''
        pht_val = TUBerlinDataset(split='val', zero_version = args.zero_version,
                                             version='ImageResized_ready',
                                             transform=transformations, aug='img', cid_mask = True)
        '''
    ...
    elif args.dataset == 'shoe': #其实是鞋子

        print('xiezi')
        end=time.time()
        skt_train = ShoeDataset(args,mode='Train',pic='sketchy')

        pht_train = ShoeDataset(args,mode='Train',pic='photo')

        skt_val = ShoeDataset(args,mode='Test',pic='sketchy')
    ...
    #for i in range(len(skt_train)):
     #   data = skt_train[i]
      #  print(f'{data[0]}: {type(data[0])}, {data[1]}: {type(data[1])}')
    ...
    return mix_loader, val_loader,val_loader1
